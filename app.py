...
def top10Camera():
    topcamera= Product.query.order_by(Product.main_camera.desc()).limit(10).all()
...
def top10cpu():
    topcpu = Product.query.join(cpu,cpu.cpu_id == Product.cpu_id).order_by(cpu.cpu_score.desc()).limit(10).all()
